...
static struct timer_list ts_timer;     //定义一个定时器
	...
	else
	{
		...
		if(cnt == 4)
		{
			if(s3c_filter_ts(x, y))
			{
				...
				input_report_abs(s3c_ts_dev, ABS_X, (x[0]+x[1]+x[2]+x[3])/4);  //x方向
				input_report_abs(s3c_ts_dev, ABS_Y, (y[0]+y[1]+y[2]+y[3])/4);  //y方向
				input_report_abs(s3c_ts_dev, ABS_PRESSURE, 1);    //1代表按下
				input_report_key(s3c_ts_dev, BTN_TOUCH, 1);       //1代表按下
				input_sync(s3c_ts_dev);
			}
			cnt = 0;
			enter_wait_pen_up_mode(); 
