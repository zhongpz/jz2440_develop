...
static int is_mylog_empty_for_read(void)   //用于read函数
{
...
static int mylog_getc_for_read(char *p)  //用于读函数
{ 
	if(is_mylog_empty_for_read())
	{
		return 0;
	}
	*p = mylog_buff[mylog_r_for_read];
			 ...
			 size_t count, loff_t *ppos)
{
	...
	if ((file->f_flags & O_NONBLOCK) && is_mylog_empty_for_read()) //非阻塞，且无数据
		return -EAGAIN;  //立即返回
		
 	err = wait_event_interruptible(mymsg_waitq, !is_mylog_empty_for_read());  //休眠等待,0休眠，1唤醒

	/* copy_to_user */
	while (!err && (mylog_getc_for_read(&c)) && i < count) {
