...
int main(int argc, char **argv)
{
	 ...
	 */
	wchar_t *chinese_str = L"钟";   //要显示的字符
 	FT_Library    library;   //freetype库变量,用于初始化
	FT_Face       face;      //用于存放加载的字体文件
	FT_Vector     pen;       
	FT_GlyphSlot  slot;      //存放字体文件中的FT_GlyphSlot
	int font_size = 24;       //字符默认大小
	int           error;
	int           fd_fb;

	FT_Matrix     matrix;     //矩阵，用来旋转字体
	double        angle;      //旋转的角度
	...
	if(argc == 4)
	{
		font_size = strtoul(argv[3], NULL, 0);  //字符串转为无符号10进制长整形
	}
	...
	/* 2.6变形 */
	pen.x = 0;   //必须设置，不然后面加载失败
	pen.y = 0;
	FT_Set_Transform(face, &matrix, &pen);   

	/* 2.7加载字符，使用Unicode值从字体文件中加载字符的位图 
