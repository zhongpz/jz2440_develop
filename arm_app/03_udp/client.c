...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		{
			/*发送数据,使用sendto包含了服务器地址参数，可以不使用connect函数*/
			iSendLen = sendto(iSocketClient, ucSendBuff, strlen(ucSendBuff), 0, (struct sockaddr *)&tSocketServerAddr, sizeof(struct sockaddr));
			if(iSendLen <= 0)
			{
				close(iSocketClient);
				return -1;
